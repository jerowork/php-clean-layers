 ...
 * @internal
 */
final class OnlyBeAllowedByRuleProcessorTest extends TestCase
{
    private OnlyBeAllowedByRuleProcessor $processor;

    protected function setUp(): void
    {
        parent::setUp();

        $this->processor = new OnlyBeAllowedByRuleProcessor();
    }

    /**
     * @test
     */
    public function itShouldNotHandleIfRuleIsNotTypeOfOnlyBeAllowedBy(): void
    {
     ...
     * @test
     */
    public function itShouldReturnNoViolationWhenOnlyClassesAllowedUseClass(): void
    {
        $classDependencies1 = new ClassDependencies('Another/Layer/Class.php');
        $classDependencies1->setFqn(new Fqn('Another\Layer\Class'));
        $classDependencies1->addInlineFqn(new Fqn('Some\Layer\Sub\Class'));
        ...
        $result = $this->processor->handle(
            new OnlyBeAllowedBy('Third\Layer', 'Fourth\Layer'),
            'Some\Layer\Class',
    ...
    public function itShouldReturnViolationsWhenSomeClassesNotAllowedUseClass(): void
    {
        ...
        self::assertEquals([
            new Violation('<fg=yellow>Another\Layer\Class</> is not allowed to use <fg=green>Some\Layer\Class</> (OnlyBeAllowedBy)'),
        ], $result);
    }
}
