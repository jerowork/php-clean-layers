...
final class NotBeAllowedByRuleProcessor implements RuleProcessor
{
    use ClassIsPartOfLayerTrait;

    private const VIOLATION_MESSAGE = '<fg=yellow>%s</> is not allowed to use <fg=green>%s</> (NotBeAllowedBy)';

    public function handle(Rule $rule, string $class, array $classDependencies): array
    {
        if (!$rule instanceof NotBeAllowedBy) {
            return [];
        }
        ...
        foreach ($classDependencies as $dependencies) {
            ...
            // When class dependency is not part of layer, discard
